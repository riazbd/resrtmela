 ...
 *
 * Named `monthOf` while it lived in the console, and renamed on the way into
 * this package: `calendar-month.ts` has a different function of the same name —
 * that one truncates a date to its month, this one does arithmetic on one — and
 * a package with a single front door cannot export both. The console keeps
 * calling it `monthOf` through an alias in its own `lib/resort-dates.ts`, so no
 * call site there changed. Which of the two names was the honest one is not a
 * close question.
 */
...
shiftMonth.lastDay = (month: string): string => {
  const [y, m] = month.split("-").map(Number);
...
shiftMonth.firstDay = (month: string): string => `${month}-01`;
