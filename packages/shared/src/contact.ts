 ...
 *
 * `isPlaceholderEmail` / `isPlaceholderPhone` come from `./placeholders`, the
 * one definition the API, the console and the app all read, so a loaded row can
 * show "not set" instead of the fake address underneath it. They are imported
 * from that module rather than from this package's own barrel, which exports
 * this file: a member importing its own barrel is a cycle.
 */
import { isPlaceholderEmail, isPlaceholderPhone } from "./placeholders";
