 ...
 *
 * The store it writes to is handed in, and deliberately *not* a
 * `guardedStorage`. This class already has a better answer to a full quota than
 * the guard's — drop the oldest half and try once more — and a guard that
 * swallowed the throw would have quietly retired that strategy while leaving
 * the code that implements it sitting there looking correct.
 */
...
export class CacheStore {
  ...
  private write(entries: Record<string, Entry>) {
    try {
      ...
      try {
        ...
        this.storage.setItem(KEY, JSON.stringify(Object.fromEntries(kept)));
      } catch {
   ...
   *
   * `removeItem` on this one key, never `storage.clear()`. The store it was
   * handed is the whole application's — the session token, the language and
   * the active resort live in it too — and clearing all of that is not what
   * emptying a cache means.
   */
  clear(): void {
    try {
