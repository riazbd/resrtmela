...
/**
 * `storage` is where the choice is remembered — localStorage on the console,
 * MMKV on the phone. `null` keeps nothing, which is what a server render is:
 * the language still works, it just opens at the default every time.
 *
 * The read is in an effect rather than in the initial state so that a server
 * render and the browser's first paint agree. Reading storage during render
 * would produce English on the server and Bangla in the browser, and React
 * would tear the page down over the mismatch.
 */
export function LangProvider({
  ...
  useEffect(() => {
    const saved = storage?.getItem("rh.lang");
