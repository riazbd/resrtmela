...
import { ROLE, type JwtClaims, type PublishedResort, type PublishedVacancy } from "@rh/shared";
...
export class V1Service {
  ...
  /**
   * The resort this key belongs to, having checked what it may do.
   *
   * An agency's key is refused here by name: it is a real key, and telling its
   * holder which door it opens is the useful answer, where a bare 401 would
   * send them looking for a typo.
   */
  private must(caller: ApiCaller, scope: "read" | "write"): number {
    if (caller.resortId == null) {
      throw forbid("This is an agency's key. An agency's website uses /v1/agency.");
    }
    if (!this.keys.may(caller, scope)) {
      throw forbid(`This key may only ${caller.scopes.join(" and ")}.`);
    }
    return caller.resortId;
  }
  ...
  async resort(caller: ApiCaller): Promise<PublishedResort> {
    return this.published.forApi(this.must(caller, "read"));
  }

  async vacancy(caller: ApiCaller, from: string, to: string): Promise<PublishedVacancy[]> {
    return this.published.vacancyFor(this.must(caller, "read"), from, to);
  }
  ...
  async book(caller: ApiCaller, idempotencyKey: string, order: V1BookingOrder): Promise<V1Booking> {
    const resortId = this.must(caller, "write");
    ...
    const before = await this.prisma.booking.findFirst({
      where: { resortId: resortId, idempotencyKey: key },
    });
    if (before) {
      ...
      return this.render(before.code, resortId);
    }

    const type = await roomTypeByKey(this.prisma, resortId, order.roomType);
    const roomId = await freeRoomOfType(this.prisma, resortId, type.id, order.checkIn, order.checkOut);
    if (roomId == null) {
      throw Object.assign(
    ...
    try {
      ...
      return this.render(made.code, resortId);
    } catch (e) {
      ...
      if ((e as { code?: string }).code === "P2002") {
        ...
        if (won) return this.render(won.code, resortId);
      }
      throw e;
    }
  }
  ...
  async booking(caller: ApiCaller, code: string): Promise<V1Booking> {
    return this.render(code, this.must(caller, "read"));
  }
  ...
  async cancel(caller: ApiCaller, code: string): Promise<V1Booking> {
    ...
    const row = await this.prisma.booking.findFirst({
      where: { code, resortId: resortId, deletedAt: null },
      select: { id: true },
    });
    ...
    return this.render(code, resortId);
  }

  /** A booking as `/v1` describes one: what the caller asked for, and nothing else. */
  async render(code: string, resortId: number): Promise<V1Booking> {
    const row = await this.prisma.booking.findFirst({
